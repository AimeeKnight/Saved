...
class UserFriendshipsControllerTest < ActionController::TestCase
			...
			context "successfully" do
				should "create two user friendship objects" do
					assert_difference "UserFriendship.count", 2 do
						post :create, user_friendship: { friend_id: users(:jay).profile_name }
					end
				end
			end
	...
	context "#accept" do
		...
		context "when logged in" do
			setup do
				@friend = create(:user)
				@user_friendship = create(:pending_user_friendship, user: users(:aimee), friend: @friend)
				create(:pending_user_friendship, friend: users(:aimee), user: @friend)
				sign_in users(:aimee)
	...
	context "#edit" do
		...
		context "when logged in" do
			setup do
				@user_friendship = create(:pending_user_friendship, user: users(:aimee))
				sign_in users(:aimee)
			...
			should "assign to user_frienship" do
				assert assigns(:user_friendship)
			end
	...
	context "#destroy" do
		...
		context "when logged in" do
			setup do
				@friend = create(:user)
				@user_friendship = create(:accepted_user_friendship, friend: @friend, user: users(:aimee))
				create(:accepted_user_friendship, friend: users(:aimee), user: @friend)
			...
			should "set the flash" do
				delete :destroy, id: @user_friendship
				assert_equal "Friendship destroyed", flash[:sucess]
			end
		end
	end
